...
for dataset in cm_datasets:
  endpoint = wdApiBaseUrl +  "datasets/" + dataset + "/rebuild" + commonParam + "&rebuild_collections=true"
  r = requests.post(endpoint, verify=False)
  if r.status_code != requests.codes.no_content:
    print(r.text)
    print("rebuild " + dataset + " returns " + r.status_code)
