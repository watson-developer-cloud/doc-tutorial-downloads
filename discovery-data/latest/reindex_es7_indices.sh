...
CHECK_ONLY=${CHECK_ONLY:-false} # If this is true, this script will not reindex any indices. It just print indices versions.
ELASTIC_STATUS_CHECK_INTERVAL=${ELASTIC_STATUS_CHECK_INTERVAL:-10}
ELASTIC_MAX_WAIT_RECOVERY_SECONDS=${ELASTIC_MAX_WAIT_RECOVERY_SECONDS:-600}

function show_help() {
    cat <<EOS
Reindex ElasticSearch indices.
If ElasticSearch indices are detected, this script will perform reindexing to update the indices as OpenSearch 2.

Usage: $0 [options]

Options:
  -h, --help                           Print help info.
  --check-only                         [Optional] If specified, this script does not perform reindexing. It will just print detected indices versions to stdout.
                                       Use this option if you only want to check your open search indices versions.

Exit Codes:
  0                                    Success
  1                                    General Error
  2                                    Check failure. Run without --check-only to reindex ElasticSearch index
EOS
}


# Get the OpenSearch cluster health status. If healthy, this should return "green".
function get_opensearch_health_status() {
...
function wait_for_index_health() {
  local index=$1
  local deadline_sec=${2:-$ELASTIC_MAX_WAIT_RECOVERY_SECONDS}
  local interval_sec=${3:-$ELASTIC_STATUS_CHECK_INTERVAL}
  local elapsed_sec=0
  local start_time=$(date +%s)

  echo "Waiting until index becomes green or yellow status ..."
  while [ ${elapsed_sec} -lt ${deadline_sec} ]; do
    # Ignore error since we retry some times and timeout
    status=$(curl -sSk -u "${ELASTIC_USER}:${ELASTIC_PASSWORD}" "${ELASTIC_ENDPOINT}/_cat/indices?index=${index}" | cut -d' ' -f1)
  ...
  echo "The index status did not become green or yellow after ${deadline_sec} sec."
}
...
function clone_index() {
    ...
    wait_for_index_health ${dst_index}
...
if [[ ${CHECK_ONLY,,} = "true" ]]; then
    echo "'--check-only' specified. Just check if ElasticSearch indices exist. No reindex will be performed."
fi
...
echo "Checking status of OpenSearch"
...
if [ "${index_count}" = "0" ] ; then
    echo "OpenSearch has no index"
    echo "Completed!"
    exit 0
fi

distribution="$(curl -sSkf -u "${ELASTIC_USER}:${ELASTIC_PASSWORD}" "${ELASTIC_ENDPOINT}" | jq -r .version.distribution)"
version_num="$(curl -sSkf -u "${ELASTIC_USER}:${ELASTIC_PASSWORD}" "${ELASTIC_ENDPOINT}" | jq -r .version.number)"

if [ "${distribution}" = "opensearch" ] ; then
    if [[ ${version_num} == 3* ]]; then
        echo "OpenSearch 3 is already used"
        exit 0
    fi
else
    echo "Unknown opensearch distribution: ${distribution}"
    exit 1
fi

echo "Getting index list"
indices="$(curl -sSkf -u "${ELASTIC_USER}:${ELASTIC_PASSWORD}" "${ELASTIC_ENDPOINT}/_cat/indices?expand_wildcards=all&h=index")"
...
for index in ${indices} ; do
    ...
    version="$(curl -sSkf -u "${ELASTIC_USER}:${ELASTIC_PASSWORD}" "${ELASTIC_ENDPOINT}/${index}/_settings?human" | jq -r .[].settings.index.version.created_string)"
    if [[ ${version} = 2* ]] ; then
        echo "[${count} / ${total}] Skip OpenSearch 2 index: ${index}"
    elif [[ ${version} = 6* ]] ; then
        echo "[${count} / ${total}] Skip ElasticSearch 6 index: ${index}"
    elif [[ ${version} = 7* ]] ; then
        if [[ ${CHECK_ONLY,,} = "true" ]]; then
            echo "[${count} / ${total}] Detected ElasticSearch 7 index: ${index}"
            trap 0 1 2 3 15
        else
            echo "[${count} / ${total}] ElasticSearch 7 index found: ${index}"
            echo "----------------------------"
        ...
        echo "Failed to get version of index or unsupported index version: ${index}"
        exit 1
    fi
    ((count++))
done
