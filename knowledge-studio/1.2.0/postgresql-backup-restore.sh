...
set_pgpass() {
  ...
  chmod 600 ${PG_PASS}
  kubectl ${KUBECTL_ARGS} cp ${PG_PASS} ${PG_POD}:/var/lib/pgsql
  rm ${PG_PASS}
}
...
SQL_PASSWORD=`kubectl ${KUBECTL_ARGS} get secret $RELEASE_NAME-edb-postgresql-auth-secret --template '{{.data.pg_su_password}}' | base64 --decode`
DBNAME=postgresql
...
PG_POD=`kubectl ${KUBECTL_ARGS} get pods -o jsonpath='{.items[0].metadata.name}' -l component=stolon-keeper | head -n 1`
...
if [ ${COMMAND} = 'backup' ] ; then
  ...
  kubectl ${KUBECTL_ARGS} exec ${PG_POD} -- pg_dump -h ${PG_POD} -p 5432 -U enterprisedb --clean -Fc jobq_${RELEASE_NAME_UNDERSCORE} > ${BACKUP_DIR}/jobq_${RELEASE_NAME_UNDERSCORE}.custom
  cmd_check
  
  echo "run pg_dump (2/4):"
  kubectl ${KUBECTL_ARGS} exec ${PG_POD} -- pg_dump -h ${PG_POD} -p 5432 -U enterprisedb --clean -Fc model_management_api > ${BACKUP_DIR}/model_management_api.custom
  cmd_check
  
  echo "run pg_dump (3/4)"
  kubectl ${KUBECTL_ARGS} exec ${PG_POD} -- pg_dump -h ${PG_POD} -p 5432 -U enterprisedb --clean -Fc model_management_api_v2 > ${BACKUP_DIR}/model_management_api_v2.custom
  cmd_check
  
  echo "run pg_dump (4/4)"
  kubectl ${KUBECTL_ARGS} exec ${PG_POD} -- pg_dump -h ${PG_POD} -p 5432 -U enterprisedb --clean -Fc awt > ${BACKUP_DIR}/awt.custom
  cmd_check
  ...
  kubectl ${KUBECTL_ARGS} exec ${PG_POD} -- pg_restore -Fc --clean -d jobq_${RELEASE_NAME_UNDERSCORE} -h ${PG_POD} -p 5432 -U enterprisedb /tmp/jobq_${RELEASE_NAME_UNDERSCORE}.custom
  cmd_check
  ...
  kubectl ${KUBECTL_ARGS} exec ${PG_POD} -- pg_restore -Fc --clean -d model_management_api -h ${PG_POD} -p 5432 -U enterprisedb /tmp/model_management_api.custom
  cmd_check
  ...
  kubectl ${KUBECTL_ARGS} exec ${PG_POD} -- pg_restore -Fc --clean -d model_management_api_v2 -h ${PG_POD} -p 5432 -U enterprisedb /tmp/model_management_api_v2.custom
  cmd_check
  
  echo "run pg_restore (3/4)"
  kubectl ${KUBECTL_ARGS} cp ${BACKUP_DIR}/awt.custom ${PG_POD}:/tmp/awt.custom
  kubectl ${KUBECTL_ARGS} exec ${PG_POD} -- pg_restore -Fc --clean -d awt -h ${PG_POD} -p 5432 -U enterprisedb  /tmp/awt.custom
  cmd_check
