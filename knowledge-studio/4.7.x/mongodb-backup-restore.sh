...
get_master_pod() {
  for pod in `oc ${KUBECTL_ARGS} get pods -o=go-template --template='{{range $pod := .items}}{{range .status.containerStatuses}}{{if .ready}}{{$pod.metadata.name}}{{"\n"}}{{end}}{{end}}{{end}}' | grep ${RELEASE_NAME}-ibm-mongodb`
  do 
    IS_MASTER_FLAG=`oc ${KUBECTL_ARGS} exec -it $pod -- mongo --tls --tlsAllowInvalidCertificates  -u $MONGO_USERNAME -p $MONGO_PASSWORD --quiet --eval "db.isMaster().ismaster" | grep "true" | wc -l`
...
backupMongoDB() {
  for data in $@
  do
  WKSDATA ENVDATA escloud_sbsep
    echo "saving $data to $REMOTE_DIR_MONGO under $MONGODB_POD_NAME ..."
    oc $KUBECTL_ARGS exec -it $MONGODB_POD_NAME -- mongodump --ssl \
          --tlsInsecure -u $MONGO_USERNAME -p $MONGO_PASSWORD \
  ...
  if [[ `oc $KUBECTL_ARGS -c mongod cp $MONGODB_POD_NAME:$REMOTE_DIR_MONGO $DATA_DIR` -eq 0 ]]; then
...
retoreMongoDB() {
  ...
  for data in $@
  do
    copyBackup=`oc $KUBECTL_ARGS cp -c mongod $DATA_DIR/$data $MONGODB_POD_NAME:$REMOTE_DIR_MONGO`
  ...
  oc $KUBECTL_ARGS exec -it $MONGODB_POD_NAME -- mongorestore --drop --ssl \
        --tlsInsecure -u $MONGO_USERNAME -p $MONGO_PASSWORD \
        --authenticationDatabase admin \
...
removeTempFile() {
  ...
  oc $KUBECTL_ARGS exec -it $MONGODB_POD_NAME -- rm -rf $1
  if [[ ! $? -eq 0 ]]; then
    echo "[WARNING] remove:'$1' temp file failed"
  fi
}

declare -r MONGO_USERNAME=`oc $KUBECTL_ARGS get secret ${RELEASE_NAME}-ibm-mongodb-auth-secret --template '{{.data.user}}' | base64 --decode `
declare -r MONGO_PASSWORD=`oc $KUBECTL_ARGS get secret ${RELEASE_NAME}-ibm-mongodb-auth-secret --template '{{.data.password}}' | base64 --decode`
...
create_output_file_mongo=`oc $KUBECTL_ARGS exec -it $MONGODB_POD_NAME -- mkdir $REMOTE_DIR_MONGO`
