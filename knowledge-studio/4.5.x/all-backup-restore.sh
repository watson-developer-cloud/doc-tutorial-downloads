...
cmd_check(){
  if [ $? -ne 0 ] ; then
    reactivate
    echo "[FAIL] MongoDB,PostgreSQL,Minio $COMMAND"
    exit 1
  fi
}
...
reactivate() {
  ...
  kubectl $KUBECTL_ARGS patch --type=merge wks ${RELEASE_NAME} -p "{\"spec\":{\"global\":{\"quiesceMode\":false}}}"
  cmd_check

  kubectl $KUBECTL_ARGS patch --type=merge wks ${RELEASE_NAME} -p "{\"spec\":{\"mma\":{\"replicas\":\"${MMA_REPLICAS}\"}}}"
  cmd_check
...
deactivating() {
  ...
  kubectl $KUBECTL_ARGS patch --type=merge wks ${RELEASE_NAME} -p '{"spec":{"global":{"quiesceMode":true}}}'
  cmd_check

  kubectl $KUBECTL_ARGS patch --type=merge wks ${RELEASE_NAME} -p '{"spec":{"mma":{"replicas":0}}}'
  cmd_check
}
...
if [[ $COMMAND = "restore" ]]; then
  if [[ ! -d ${DATA_DIR_MONGODB} || ! -d ${DATA_DIR_POSTRGESQL} || ! -d ${DATA_DIR_MINIO} ]]; then
...
################################################################
jobs=`kubectl $KUBECTL_ARGS get job --no-headers | grep -e ${RELEASE_NAME}-train -e ${RELEASE_NAME}-batch-apply | awk '{print $2}'`
for job in ${jobs}; do
  if [[ $job == "0/1" ]]; then
...
POSTGRESQL_POD_NAME=`kubectl $KUBECTL_ARGS get pods -o=go-template --template='{{range $pod := .items}}{{range .status.containerStatuses}}{{if .ready}}{{$pod.metadata.name}}{{"\n"}}{{end}}{{end}}{{end}}' | grep ${RELEASE_NAME}-edb-postgresql | head -n 1`
...
POSTGRESQL_IMAGE_NAME=`kubectl $KUBECTL_ARGS get pod $POSTGRESQL_POD_NAME -o jsonpath='{.spec.containers[0].image}'`
...
################################################################
#backup/restore all MongoDB PostgreSQL Minio data
################################################################
...
echo "[SUCCESS] MongoDB,PostgreSQL,Minio $COMMAND"
