...
MINIO_SVC=`kubectl ${KUBECTL_ARGS} get svc -l release=${RELEASE_NAME}-minio,helm.sh/chart=ibm-minio -o jsonpath="{.items[*].metadata.name}" | tr '[[:space:]]' '\n' | grep headless`
TMP_FILENAME="wkstmp_`date '+%Y%m%d_%H%M%S'`"
if [ ${COMMAND} = 'backup' ] ; then
  ...
  if [[ ! -d ${BACKUP_DIR}/wks-icp ]]; then
    worrying "no data of minio to restore"
  fi
  ...
  $MC --insecure config host add ${MC_ALIAS} https://localhost:$MINIO_LPORT ${MINIO_ACCESS_KEY} ${MINIO_SECRET_KEY}
  cmd_check
  $MC --insecure cp -r ${BACKUP_DIR}/wks-icp ${MC_ALIAS}/
  cmd_check
