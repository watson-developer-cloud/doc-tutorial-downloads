...
MONGODB_POD_NAME=`kubectl ${KUBECTL_ARGS} get pods -o=go-template --template='{{range $pod := .items}}{{range .status.containerStatuses}}{{if .ready}}{{$pod.metadata.name}}{{"\n"}}{{end}}{{end}}{{end}}' | grep ${RELEASE_NAME}-ibm-mongodb | head -n 1`
cmd_check
...
declare -r MONGO_USERNAME=`kubectl $KUBECTL_ARGS get secret wks-ibm-mongodb-auth-secret --template '{{.data.user}}' | base64 --decode `
declare -r MONGO_PASSWORD=`kubectl $KUBECTL_ARGS get secret wks-ibm-mongodb-auth-secret --template '{{.data.password}}' | base64 --decode`

declare -r REMOTE_DIR_MONGO="home/mongodb/output"
# declare -r MONGODB_POD_NAME="$RELEASE_NAME-ibm-mongodb-server-0"
